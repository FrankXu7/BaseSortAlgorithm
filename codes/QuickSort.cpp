 ...
 *
 * 【基本思路】
 * 进行多轮排序，每轮在区间内任意选择一个基准数字，使得最后基准数字会大于其左侧元素，小于其右侧元素，
 ...
 * 时间复杂度O(N^2)这种情况出现概率极小极小；其次，快速排序更多的是比较操作，已经少量的赋值或交换操作，相比于
 * 归并排序大量的合并操作，效率上也更高；再者，并且快速排序是原地排序，只需要栈空间开销，而归并排序需要一个
 * 和数组同样长度的额外空间，且如果是递归实现，也需要栈空间开销。所以通常情况下，如果没有稳定性要求，首选
 * 快速排序。
 *
 * @author FrankX
 * @date 2021-07-22
